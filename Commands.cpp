			...
			else
			{
				for (int i = 1; i != msg.getFlags().size(); i++)
				{
					...
					if (!(*itchan).second->isChop(client))
						return !sendCommandResponse(client, ERR_CHANOPRIVSNEEDED,
						...
						return true; //print list
										...
										msg.getParam().size() < 3)
						return !sendCommandResponse(client, ERR_NEEDMOREPARAMS,
	...
	else if (msg.isChannel(target))
	{
		...
		if (it != _channels.end())
		{
			if (((*it).second->getFlags() & ChannelMode::NO_OUTSIDE_MSG &&
							...
							!(*it).second->isChop(client) || (*it).second->isBanned(client))
				return false;
